...
The initial base damage generated by an `ActionItem` before applying multipliers is defined as:

$$\text{BaseDamage}(\text{action}) = \text{count} \times \text{BaseWeight}(\text{actionType})$$

1. **`commit`**:
   $$\text{BaseDamage} = \text{count} \times \text{[TBD: Base Commit Damage Weight]}$$
2. **`branch_create`**:
   Initializes `branchDamagePool[branchId] = 0`.
3. **`merge_commit` / `pr_merge`**:
   $$\text{BaseDamage} = \text{branchDamagePool}[\text{branchId}] + \text{[TBD: Base Merge Damage Bonus]}$$
4. **`issue_event` / `release_published` / `deployment_event`**:
   $$\text{BaseDamage} = \text{[TBD: Event Base Damage Bonus]}$$
...
1. **Streak Multiplier Formula**:
   ...
   $$\text{StreakMultiplier} = 1.0 + \min\left(\text{currentStreak} \times \text{[TBD: Streak Multiplier Step]}, \text{[TBD: Max Streak Multiplier Cap]}\right)$$

2. **Personal Record Activity Bonuses**:
   Setting a personal daily commit peak injects flat bonus damage:
   - **Monthly Record (`record_activity_day`)**: $+ \text{[TBD: Monthly Record Damage Bonus]}$
   - **Yearly Record (`record_activity_day`)**: $+ \text{[TBD: Yearly Record Damage Bonus]}$
   - **All-Time Record (`record_activity_day`)**: $+ \text{[TBD: All-Time Record Damage Bonus]}$

3. **Combined Final Damage Equation**:
   $$\text{FinalDamage} = (\text{BaseDamage} + \text{RecordBonus}) \times \text{StreakMultiplier}$$
